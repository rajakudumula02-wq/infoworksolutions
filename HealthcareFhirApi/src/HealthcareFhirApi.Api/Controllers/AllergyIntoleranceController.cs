...
public class AllergyIntoleranceController : FhirControllerBase
{
    ...
    public AllergyIntoleranceController(
        IFhirResourceRepository<AllergyIntolerance> repo,
    ...
    public async System.Threading.Tasks.Task<IActionResult> Read(string id, CancellationToken ct)
    {
        ...
        if (resource is null)
            return FhirNotFound("AllergyIntolerance", id);
        ...
        CancellationToken ct)
    {
        ...
        var baseUrl = $"{Request.Scheme}://{Request.Host}/AllergyIntolerance";
        var bundle  = _pager.BuildSearchBundle(result.Items, parameters, result.TotalCount, baseUrl);
        return FhirOk(bundle);
    }

    [HttpPost]
    public async System.Threading.Tasks.Task<IActionResult> Create([FromBody] AllergyIntolerance resource, CancellationToken ct)
    {
        ...
        var location = $"{Request.Scheme}://{Request.Host}/AllergyIntolerance/{created.Id}";
        return FhirCreated(created, location);
    }

    [HttpPut("{id}")]
    public async System.Threading.Tasks.Task<IActionResult> Update(string id, [FromBody] AllergyIntolerance resource, CancellationToken ct)
    {
        var outcome = await _validator.ValidateAsync(resource, ProfileUrl, ct);
        if (!_validator.IsValid(outcome))
            return FhirValidationError(outcome);
